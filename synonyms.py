...
stopwords = open('resource/stopwords_small.txt', encoding='utf8').read().split('\n')
stopwords = set([w.replace(' ','_') for w in stopwords])
punct_set = set([c for c in string.punctuation]) | set(['“','”',"...","–","…","..","•",'“','”'])

with open('resource/bigram.txt', encoding='utf8') as f:
    data = f.read().split('\n')
...
map_synonym = json.load(open('resource/synonym.json'))
